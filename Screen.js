class Screen {
  ...
  enter() {
    let inc = 3;
    push();
    translate(0, -height / 2 + (height / 15) * inc, 0);
    noStroke();
    texture(getImage(5));
    plane((width / 10) * inc, (height / 15) * inc);
    translate(mybox.x, +height / 2 - (height / 15) * inc + mybox.z + getSize(true) / 2, mybox.y);
    rotateX(millis() / 10);
  ...
  score(sc, lvl) { //game score update on top
    let num = 5, //max digits of score
      inc = 1.5, //digit size increment
      nWidth = (width * 3 / 100) * inc,//number width
      sWidth = (width * 3 / 20) * inc, //"Score" width
      ht = (height / 15) * inc; //height of all
    noStroke();
    push();
    translate(mybox.x - width / 2 + sWidth / 2, -height / 2 + ht / 2, 0);
    texture(getImage(3));
  ...
  levelup(lvl) {
    ...
    let inc = 3,
        ht = (height / 15) * inc; //height of both "Level" and number
    if (lvl < 11) {
      let lWidth = (width * 11 / 75) * inc, //"Level" width
          nWidth = (width * 3 / 100) * inc; //number width
      push();
  ...
  over(score, b) { //game score
    ...
    noStroke();
    translate(0, 0, width * 49 / 60);
    this.format(getImage(i), getBase().w, getBase().h / 3);//Game Over Screen
    score = int(score);
    translate(((score + "").length) * num.w / 2 + num.w / 2, h, 0);
    while (score != 0) {
      translate(-num.w, 0, 0);
  ...
  arrow() {
    ...
    vertex(-150 * inc, -75 * inc, -50 * inc);
    vertex(0, 75 * inc, -50 * inc);
    vertex(0, -75 * inc, -50 * inc);
    vertex(0, -150 * inc, -50 * inc);
    vertex(150 * inc, 0 * inc, -50 * inc);
    vertex(0, -150 * inc, -50 * inc);
    vertex(0, 75 * inc, -50 * inc);
    vertex(150 * inc, 0, -50 * inc);
    vertex(0, 150 * inc, -50 * inc);
    vertex(0, 75 * inc, -50 * inc);
