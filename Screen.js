class Screen {
  ...
  score(sc, lvl) { //game score update on top
    ...
    noStroke();
    push();
    translate(mybox.x - width / 2 + sWidth / 2 /*mybox.x - (sWidth + (nWidth * 4)) / 2*/ , -280, 0);
    texture(getImage(3));
    ...
    if (lvl < 11) {
      ...
      push();
      translate(mybox.x + width / 2 - (lWidth / 2 + nWidth * 2), -280, 0);
      texture(getImage(6));
      ...
      push();
      translate(mybox.x + width / 2 - lWidth / 2, -280, 0);
      texture(getImage(7));
  ...
  levelup(lvl) {
    this.z = this.z + 2; //speed of movement towards the screen
    let inc = 3;
    ...
    if (lvl < 11) {
      ...
      let nWidth = (width * 3 / 100) * inc; //number width
      push();
      translate(mybox.x - (lWidth + (2 * nWidth)) / 2 + lWidth / 2, -170, this.z);
      texture(getImage(6));
      plane(lWidth, ht); //LEVEL
      translate(lWidth / 2 + nWidth * 5 / 2, 0, 0);
      lvl = int(lvl * 10);
      ...
      let lWidth = (width * 17 / 75) * inc; //"Level" width
      push();
      translate(mybox.x, -170, this.z);
      texture(getImage(7));
      plane(lWidth, ht); //FINAL LEVEL
      pop();
    } else if (lvl === 12) {
      push();
      translate(mybox.x, -170, 0);
      texture(getImage(8));
